...
@app.get("/step")
def step_browser(session_id: Optional[str] = None, type: Optional[str] = None, index: int = 0, thought: str = ""):
    """
    Allow taking a step via GET (useful for browser debugging).
    Example: /step?session_id=123&type=wait
    """
    if not session_id or not type:
        return {
            "detail": "Missing parameters",
            "usage": "/step?session_id={uuid}&type={action_type}",
            "available_actions": ["remove_item", "process_order", "handle_return", "wait", "add_item"],
            "hint": "Check /sessions for active IDs or /reset to start a new one."
        }
    return step(ActionRequest(session_id=session_id, type=type, index=index, thought=thought))
